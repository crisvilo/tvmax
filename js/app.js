...
(function () {
  ...
  async function loadAdminData(){const today=getTodayISO();const [sr,cnt,qr,dashboard,monthlySales]=await Promise.all([
    ...
    cachedQuery(`surveys:tvmax:admin:${today}`,async()=>{const r=await sbClient.from("encuestas").select("id,asesor_id,codigo_nombre_usuario,q2_servicio,observacion_q2,q3_tecnica,observacion_q3,q4_administrativa,observacion_q4,q5_agilidad,observacion_q5,q6_recomendaria,observacion_q6,q7_recomendacion,fecha_encuesta,created_at,updated_at,perfiles:asesor_id(id,nombre,apellido,email)").eq("fecha_encuesta",today).order("id",{ascending:false}).limit(10);if(r.error)throw r.error;return r.data||[];}),
    ...
    sales=sr;adminMonthlySales=monthlySales||[];advisors=[];surveys=qr;adminGoalAdvisors=Array.isArray(dashboard?.asesores)?dashboard.asesores:[];setText("advisor-count",cnt.count??0);populateAdminFilters();populateSurveyAdvisorFilter();renderAdmin();renderUsers();updateAdminDashboard(dashboard);renderSurveyReport();renderConfig();
  }
  ...
  function getAdminMonthlyGoal(){
    return adminGoalAdvisors.reduce((sum,a)=>sum+Math.max(1,Number(a.meta)||50),0);
  }
  function getAdminMonthlySales(){
    return adminMonthlySales;
  }
  function updateAdminDashboard(dashboard=null){
    ...
    const made=ventas,goal=dashboardAdvisors.reduce((sum,a)=>sum+Math.max(1,Number(a.meta)||50),0),pct=goal?Math.min(100,Math.round(made/goal*100)):0;
    setText("dash-admin-goal-title",`${made} / ${goal} operaciones`);
    setText("dash-admin-goal-period",`Meta total de ${dashboardAdvisors.length} asesor${dashboardAdvisors.length===1?"":"es"} para ${getMonthLabel()}. El avance utiliza únicamente ventas. El avance se reinicia automáticamente al cambiar de mes.`);
    ...
    id("dash-goals-list").innerHTML=dashboardAdvisors.map(a=>{const n=[a.nombre,a.apellido].filter(Boolean).join(" ")||a.email,count=adminMonthlySales.filter(s=>s.asesor_id===a.id).length,g=Math.max(1,Number(a.meta)||50),ap=Math.min(100,Math.round(count/g*100));return `<div class="goal-list-row"><div><strong>${escapeHTML(n)}</strong><small>${count} / ${g} operaciones</small></div><div class="mini-progress"><span style="width:${ap}%"></span></div><b>${ap}%</b></div>`;}).join("")||'<p class="muted">No hay asesores registrados.</p>';
  ...
  function buildReportHTML(){const filtered=getFilteredAdminSales(),total=filtered.length,ventas=filtered.filter(s=>s.tipo_operacion==="Venta").length,recon=filtered.filter(s=>s.tipo_operacion==="Reconexión").length,otros=filtered.filter(s=>s.tipo_operacion==="Otros").length,real=filtered.filter(s=>s.estado_instalacion==="REALIZADA").length,pending=filtered.filter(s=>s.estado_instalacion==="PENDIENTE").length,cancel=filtered.filter(s=>s.estado_instalacion==="CANCELADA").length,pct=n=>total?Math.round(n/total*100):0;
    ...
    const adminGoalRows=adminGoalAdvisors.map(a=>{const n=[a.nombre,a.apellido].filter(Boolean).join(" ")||a.email||"Asesor",g=Math.max(1,Number(a.meta)||50),made=Number(a.realizadas)||0,p=Math.min(100,Math.round(made/g*100));return `<tr><td>${escapeHTML(n)}</td><td>${made}</td><td>${g}</td><td>${p}%</td></tr>`;}).join("")||'<tr><td colspan="4" class="print-empty-row">No hay asesores registrados.</td></tr>';
  ...
  async function downloadExcel(){
    try{
      ...
      push(["META DEL ADMINISTRADOR · MES ACTUAL"]);push(["Indicador","Valor"]);push(["Ventas del mes",getAdminMonthlySales().length]);push(["Meta total de asesores",getAdminMonthlyGoal()]);push(["Cumplimiento",`${getAdminMonthlyGoal()?Math.min(100,Math.round(getAdminMonthlySales().length/getAdminMonthlyGoal()*100)):0}%`]);push(["Asesores incluidos",adminGoalAdvisors.length]);push([]);
      ...
      push(["GRÁFICO · CUMPLIMIENTO DE META POR ASESOR (VENTAS)"]);push(["Asesor","Operaciones","Meta","% cumplimiento"]);
